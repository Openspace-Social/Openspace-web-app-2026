...
// connections list. Capping the ListBox itself with a maxHeight ScrollView
// gives each list section its own independent scroll surface, so a long
// connections list always works regardless of what the outer wrapper does.
//
// Native gets a larger maxHeight than web because the bottom-sheet modal
// has more vertical room than web's centered floating card. nestedScroll
// is set defensively so Android also routes drag events to the inner
// ScrollView when both are in play.
...
function DetailScroller({ children }: { children: React.ReactNode }) {
  ...
  // long member/connection lists). A second outer ScrollView here would
    ...
    <View style={[s.container, { backgroundColor: c.surface, borderColor: c.border }]}>
      ...
      <Modal visible={detailOpen} transparent animationType="fade" onRequestClose={closeDetail}>
        {/* Sibling-overlay pattern: a separate Pressable above the card catches
            ...
            Wrapping the card in a Pressable (the previous approach) made the
            card's gesture handler compete with inner ScrollViews on iOS, so
            drags on member rows wouldn't scroll. */}
